(function(){
  ...
  var FIELD_DEFS={
    salesPrice:{label:'Sales Price'},
    loanAmount:{label:'Loan Amount'},
    appraisedValue:{label:'Appraised Value'},
    lenderPolicy:{label:"Lender's Title Policy"},
    ownerInsurance:{label:"Owner's Title Insurance"},
    ownerTitle:{label:"Owner's Title Quote"},
    deedTax:{label:'Transfer Taxes (Deed)'},
    mortgageTax:{label:'Transfer Taxes (Mtg)'}
  };
  ...
  function buildQuote(type){
    type=type||typeById(state.typeId);
    if(!type)return '';
    ...
    if(type.id==='loan-purchase'){
      addFee(lines,'Title - CPL Fee',50);
      addEntered(lines,"Title - Lender's Title Policy",'lenderPolicy');
      addFee(lines,'Title - Settlement Fee',450);
      addFee(lines,'Title - Title Search Fee',300);
      addFee(lines,'Recording Fee',116,'est');
      addEntered(lines,'Transfer Taxes (Deed)','deedTax');
      addEntered(lines,'Transfer Taxes (Mtg)','mortgageTax');
      addNotices(lines,type);
      lines.push('','OTHER:');
      addEntered(lines,"Title - Owner's Title Quote",'ownerTitle');
    }
    ...
    if(type.id==='refinance'){
      ...
      addNotices(lines,type);
      lines.push('');
      addFee(lines,'Recording Fee',103,'est');
      addEntered(lines,'Transfer Taxes (Mtg)','mortgageTax');
    }

    if(type.id==='reverse'){
      addFee(lines,'Closing Fee',500);
      addFee(lines,'Title Search Fee',300);
      addEntered(lines,"Lender's TI Fee",'lenderPolicy');
      addFee(lines,'CPL Fee',50);
      addFee(lines,'Mortgage Recording Fee',159,'est');
      addFee(lines,'Mortgage Tax','N/A');
      addNotices(lines,type);
    }

    if(type.id==='cash-purchase'){
      addFee(lines,'Buyer Closing Fee',175);
      addFee(lines,'Title Search Fee',300);
      addEntered(lines,"Owners Title Insurance",'ownerInsurance');
      addFee(lines,'Deed Recording Fee',13);
      addEntered(lines,'Deed Tax:','deedTax');
      addFee(lines,'Seller Closing Fee',175);
      if(state.decisions.payoffNeeded==='yes')addFee(lines,'Payoff fee (if needed)',50);
      addFee(lines,'Deed Preparation Fee',150);
      addNotices(lines,type);
    }
    ...
    if(type.id==='apex-purchase'){
      addFee(lines,'Title - CPL Fee',50);
      addEntered(lines,"Title - Lender's Title Policy",'lenderPolicy');
      addFee(lines,'Title - Settlement Fee',400);
      addFee(lines,'Title - Title Search Fee',250);
      addFee(lines,'Recording Fee',116,'est');
      addEntered(lines,'Transfer Taxes (Deed)','deedTax');
      addEntered(lines,'Transfer Taxes (Mtg)','mortgageTax');
    ...
    if(type.id==='apex-refinance'){
      addFee(lines,'Title - CPL Fee',50);
      addEntered(lines,"Title - Lender's Title Policy",'lenderPolicy');
      addFee(lines,'Title - Settlement Fee',350);
      addFee(lines,'Title - Title Search Fee',250);
      addNotices(lines,type);
      lines.push('');
      addFee(lines,'Recording Fee',103,'est');
    ...
    if(type.id==='ccu-hcb-purchase'){
      if(state.decisions.cplRequired==='yes')addFee(lines,'Title - CPL Fee',50);
      if(state.decisions.lenderPolicyRequired==='yes')addEntered(lines,"Title - Lender's Title Policy",'lenderPolicy');
      addFee(lines,'Title - Settlement Fee',450);
      addFee(lines,'Title - Title Search Fee',300);
      addFee(lines,'Recording Fee',116,'est');
      addEntered(lines,'Transfer Taxes (Deed)','deedTax');
    ...
    if(type.id==='ccu-hcb-refinance'){
      if(state.decisions.cplRequired==='yes')addFee(lines,'Title - CPL Fee',50);
      if(state.decisions.settlementRequired==='yes')addFee(lines,'Title - Settlement Fee',400);
      if(state.decisions.lenderPolicyRequired==='yes')addEntered(lines,"Title - Lender's Title Policy",'lenderPolicy');
      addFee(lines,'Title - Title Search Fee',300);
      addNotices(lines,type);
      lines.push('');
  ...
  function renderTransactions(){
    ...
    var groups=['Standard','In-House Lenders','Other'];
    grid.innerHTML=groups.map(function(group){
      var items=TYPES.filter(function(type){return type.group===group;});
      if(!items.length)return '';
      return '<div class="transaction-group"><div class="transaction-group-title">'+escapeHtml(group)+'</div><div class="transaction-grid">'+items.map(function(type){
        return '<button type="button" class="transaction-option'+(type.inHouse?' in-house':'')+'" aria-pressed="'+(type.id===state.typeId)+'" data-type="'+type.id+'"><strong>'+escapeHtml(type.name)+'</strong></button>';
      }).join('')+'</div></div>';
    }).join('');

    grid.querySelectorAll('[data-type]').forEach(function(button){
      button.addEventListener('click',function(){selectType(button.dataset.type);});
    });
  }
  ...
  function selectType(id){
    ...
    if(window.matchMedia('(max-width: 760px)').matches)workspace.scrollIntoView({behavior:'smooth',block:'start'});
  }

  function renderDecision(key){
    var def=DECISIONS[key],current=state.decisions[key]||'';
    return '<div class="decision-group"><span>'+escapeHtml(def.label)+'</span><div class="choice-row">'+def.choices.map(function(choice){
      return '<button type="button" class="choice-button" data-decision="'+key+'" data-value="'+choice[0]+'" aria-pressed="'+(current===choice[0])+'">'+escapeHtml(choice[1])+'</button>';
    }).join('')+'</div></div>';
  }

  function fieldLabel(type,key){
    if(type.id==='ccu-hcb-purchase'&&key==='ownerTitle'){
      if(state.decisions.ownerPolicyType==='simultaneous')return "Owner's Title Quote - simultaneous issue";
      if(state.decisions.ownerPolicyType==='standard')return "Owner's Title Quote - standard";
    }
  ...
  function renderForm(){
    ...
    document.getElementById('detailsHelp').textContent=type.id==='apex-refinance'
      ?'Choose what Unified is handling, then enter the amounts for that path.'
      :'Enter the confirmed amounts for this file.';
    ...
    document.querySelectorAll('[data-decision]').forEach(function(button){
      button.addEventListener('click',function(){
        var key=button.dataset.decision;
        state.decisions[key]=button.dataset.value;

        if(key==='apexClosing'){
          delete state.decisions.apexTitleInsurance;
          delete state.values.lenderPolicy;
          delete state.values.mortgageTax;
        }
        if(key==='lenderPolicyRequired'&&button.dataset.value==='no')delete state.values.lenderPolicy;
    ...
    document.getElementById('amountFields').innerHTML=fields.map(function(key){
      return '<div class="field"><label for="field-'+key+'">'+escapeHtml(fieldLabel(type,key))+'</label><div class="money-wrap"><span class="money-prefix">$</span><input id="field-'+key+'" name="'+key+'" inputmode="decimal" autocomplete="off" placeholder="0.00" value="'+escapeHtml(state.values[key]||'')+'"></div></div>';
    }).join('');

    document.querySelectorAll('#amountFields input').forEach(function(input){
      input.addEventListener('input',function(){
        var clean=input.value.replace(/[^0-9.]/g,'');
        var parts=clean.split('.');
        if(parts.length>2)clean=parts.shift()+'.'+parts.join('');
        input.value=clean;
        state.values[input.name]=clean;
  ...
  function updatePreview(){
    ...
    var missing=missingItems(type);
    var pill=document.getElementById('completionPill');
    pill.textContent=missing.length?'Needs '+missing.length:'Ready';
    pill.classList.toggle('complete',!missing.length);
  ...
  function validate(){
    ...
    var missing=missingItems(type);
    document.querySelectorAll('#amountFields input').forEach(function(input){
      input.classList.toggle('invalid',!validAmount(input.value));
    });

    var alert=document.getElementById('formAlert');
    if(missing.length){
      alert.textContent='Finish the missing quote details before copying or printing.';
      alert.hidden=false;
  ...
  function resetQuote(){
    state={typeId:'',values:{},decisions:{}};
    renderTransactions();
    document.getElementById('builderWorkspace').hidden=true;
    document.getElementById('quoteForm').reset();
    document.getElementById('actionStatus').textContent='';
    document.getElementById('transactionHeading').scrollIntoView({behavior:'smooth',block:'start'});
  }
  ...
  if(typeof module!=='undefined'&&module.exports){
    module.exports={TYPES:TYPES,NOTICES:NOTICES,typeById:typeById,money:money,_setState:function(next){state=next;},buildQuote:buildQuote,missingItems:missingItems};
  }
