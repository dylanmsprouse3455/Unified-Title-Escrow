...
test('standard purchase, refinance, reverse, and cash fixed fees match the guide',()=>{
  ...
  assert.match(refinance,/\$450\.00/);
  assert.match(refinance,/\$300\.00/);
  assert.match(refinance,/\$103\.00/);
  assert.doesNotMatch(refinance,/Transfer Taxes \(Deed\)/);

  const reverse=render('reverse');
  ['$500.00','$300.00','$50.00','$159.00'].forEach(amount=>assert.ok(reverse.includes(amount)));
  assert.match(reverse,/Mortgage Tax\s+N\/A/);
...
test('APEX in-house fees and title-only branches match the guide',()=>{
  ...
  assert.match(titleOnly,/Title Search Fee\s+\$250\.00/);
  assert.match(titleOnly,/Lender's Title Policy\s+\$875\.00/);
  assert.doesNotMatch(titleOnly,/Settlement Fee/);

  const noInsurance=render('apex-refinance',{}, {apexClosing:'title-only',apexTitleInsurance:'no'});
  assert.match(noInsurance,/Final Update Fee\s+\$50\.00/);
  assert.doesNotMatch(noInsurance,/Lender's Title Policy/);
});
...
test('CCU and HCB optional fees appear only when required',()=>{
  ...
  assert.doesNotMatch(refinance,/CPL Fee|Settlement Fee|Lender's Title Policy/);
...
test('FSBO includes buyer and seller fees and conditionally includes payoff',()=>{
  ...
  assert.match(withPayoff,/BUYER FEES/);
  assert.match(withPayoff,/SELLER FEES/);
  assert.match(withPayoff,/Payoff fee \(if needed\)\s+\$50\.00/);
  assert.match(withPayoff,/Seller Closing Fee\s+\$175\.00/);
  assert.match(withPayoff,/Deed Preparation Fee\s+\$150\.00/);
...
test('CCU/HCB purchase requires the owner policy path before completion',()=>{
...
test('the PDF notices are preserved on their applicable quote types',()=>{
  const refinance=render('refinance');
  assert.ok(refinance.includes(quote.NOTICES.documentPreparation));
  assert.ok(refinance.includes(quote.NOTICES.manufacturedHome));
  assert.ok(refinance.includes(quote.NOTICES.remoteNotary));

  const cash=render('cash-purchase',{}, {payoffNeeded:'no'});
  assert.ok(cash.includes(quote.NOTICES.manufacturedHome));
  assert.ok(!cash.includes(quote.NOTICES.remoteNotary));
  assert.ok(!cash.includes(quote.NOTICES.documentPreparation));
});

test('copied output does not include internal in-house or workflow commentary',()=>{
  const apex=render('apex-purchase');
  assert.doesNotMatch(apex,/IN-HOUSE LOANS ONLY/);
  const titleOnly=render('apex-refinance',{lenderPolicy:'875'},{apexClosing:'title-only',apexTitleInsurance:'yes'});
