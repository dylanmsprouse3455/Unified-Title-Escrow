...
function printAllRecords(){
  if(!calls.length){alert("There are no case records to print.");return;}
  var ordered=sortCalls(calls);
  var sheets=ordered.map(function(r){
    var cd=countdown(r),todos=ensureTodos(r),openTodos=todos.filter(function(todo){return !todo.completed;}).length;
    var todoItems=todos.length?todos.map(function(todo){
      return'<li class="'+(todo.completed?'done':'')+'"><span class="check">'+(todo.completed?'✓':'')+'</span><div><strong>'+esc(todo.text)+'</strong><small>'+(todo.completed?'Completed':'Open task')+'</small></div></li>';
    }).join(""):'<li class="empty">No tasks recorded.</li>';
    var followDate=r.followUpDate?localDate(r.followUpDate):"No date set";
    return'<section class="case-page"><header class="masthead"><div><div class="office">Unified Title &amp; Escrow</div><h1>'+esc(r.fileNumber||"No File Number")+'</h1><strong>'+esc(r.address||"No property address entered")+'</strong></div><div class="status"><span>Status</span><strong>'+esc(r.status||"Open")+'</strong></div></header><div class="grid"><div class="box"><span class="label">Caller</span><div class="value">'+esc(r.caller||"Not entered")+'</div></div><div class="box"><span class="label">Callback Number</span><div class="value">'+esc(r.phone||"Not entered")+'</div></div><div class="box"><span class="label">Company / Role</span><div class="value">'+esc(r.companyRole||"Not entered")+'</div></div><div class="box"><span class="label">Assigned To</span><div class="value">'+esc(r.assignedTo||"Not entered")+'</div></div><div class="box"><span class="label">Follow-Up</span><div class="value">'+esc(followChoiceLabel(r.followUpType))+' · '+esc(followDate)+' · '+esc(cd.label)+'</div></div><div class="box"><span class="label">Last Updated</span><div class="value">'+esc(localDateTime(r.updatedAt||r.createdAt))+'</div></div><div class="box wide"><span class="label">Reason / Issue</span><div class="value">'+esc(r.issueType||"Not entered")+'</div></div></div><section class="narrative"><h2>Why They Called</h2><p>'+esc(r.notes||"Not entered")+'</p></section><section class="narrative"><h2>What I Advised</h2><p>'+esc(r.promise||"Not entered")+'</p></section><section class="follow"><div><h2>Next Action</h2><strong>'+esc(r.nextAction||r.promise||"No next action entered")+'</strong></div><div><h2>Final Outcome</h2><strong>'+esc(r.finalOutcome||"Not completed")+'</strong></div></section><section class="tasks"><div class="section-head"><h2>Case To-Do List</h2><strong>'+openTodos+' open</strong></div><ul>'+todoItems+'</ul></section><footer>Case Workspace · Dylan</footer></section>';
  }).join("");
  var w=window.open("","utei-all-call-sheets","width=980,height=940");
  if(!w){alert("Please allow pop-ups to open the full case packet.");return;}
  w.document.write('<!doctype html><html><head><meta charset="utf-8"><title>All Case Workspace Records</title><style>@page{size:letter;margin:.48in}*{box-sizing:border-box}html{background:#ddd}body{width:min(8.5in,100%);margin:18px auto;background:#fff;color:#000;font-family:Arial,Helvetica,sans-serif;font-size:10.5pt;line-height:1.38}.preview-actions{position:fixed;right:20px;bottom:20px;z-index:3;display:flex;gap:9px}.preview-actions button{padding:11px 16px;border:2px solid #000;border-radius:5px;background:#fff;color:#000;font:700 10pt Arial;cursor:pointer}.preview-actions .print{background:#000;color:#fff}.case-page{min-height:10in;padding:.48in;page-break-after:always}.case-page:last-of-type{page-break-after:auto}.masthead{display:flex;justify-content:space-between;gap:18px;padding-bottom:12px;border-bottom:4px solid #000}.office{font-size:8.5pt;font-weight:900;letter-spacing:.1em;text-transform:uppercase}.masthead h1{margin:3px 0;font-size:22pt}.masthead>div>strong{font-size:12pt}.status{min-width:125px;padding:9px 11px;border:2px solid #000;border-radius:7px}.status span,.label{display:block;margin-bottom:3px;font-size:7.5pt;font-weight:900;letter-spacing:.07em;text-transform:uppercase}.status strong{font-size:13pt}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:11px}.box{padding:8px 10px;border:1px solid #777;border-radius:7px;break-inside:avoid}.box.wide{grid-column:1/-1}.value{font-weight:700;white-space:pre-wrap}.narrative{margin-top:9px;padding:10px 12px;border:1px solid #777;border-left:5px solid #000;break-inside:avoid}.narrative h2,.follow h2,.tasks h2{margin:0 0 4px;font-size:8.5pt;font-weight:900;letter-spacing:.06em;text-transform:uppercase}.narrative p{margin:0;white-space:pre-wrap}.follow{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:9px}.follow>div{padding:10px 12px;border:2px solid #000;border-radius:7px;break-inside:avoid}.follow strong{display:block;font-size:11pt;white-space:pre-wrap}.tasks{margin-top:10px;padding:10px 12px;border:2px solid #000;border-radius:7px;break-inside:avoid}.section-head{display:flex;align-items:baseline;justify-content:space-between;gap:12px}.section-head>strong{font-size:8.5pt}.tasks ul{margin:4px 0 0;padding:0;list-style:none}.tasks li{display:grid;grid-template-columns:22px 1fr;gap:8px;padding:6px 0;border-top:1px solid #aaa;break-inside:avoid}.tasks li:first-child{border-top:0}.check{width:17px;height:17px;display:grid;place-items:center;border:2px solid #000;border-radius:2px;font-weight:900}.tasks li.done strong{text-decoration:line-through;color:#555}.tasks li strong{display:block}.tasks li small{display:block;color:#555;font-size:8pt}.tasks li.empty{display:block;color:#555}footer{margin-top:10px;padding-top:6px;border-top:1px solid #777;font-size:8pt}@media print{html{background:#fff}body{width:auto;margin:0}.case-page{padding:0}.preview-actions{display:none}}</style></head><body>'+sheets+'<div class="preview-actions"><button type="button" onclick="window.close()">Close Preview</button><button type="button" class="print" onclick="window.print()">Print All '+ordered.length+' Cases</button></div></body></html>');
  w.document.close();
  w.focus();
}
